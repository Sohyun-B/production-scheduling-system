...
class FabricRuleHandler:
    ...
    def _handle_type0(self, sub_df: pd.DataFrame) -> dict:
        ...
        return {
            'fabric_width': 1500,
            'production_length': sub_df['생산길이'].sum()
        }
    ...
    def _handle_type1(self, sub_df: pd.DataFrame) -> dict:
        ...
        """
        # 필수 원단 존재 여부 확인
        has_508 = 508 in sub_df['너비'].values
        has_1016 = 1016 in sub_df['너비'].values
        ...
        # 조합 처리
        len_508 = sub_df.loc[sub_df['너비'] == 508, '생산길이'].sum()
        len_1016 = sub_df.loc[sub_df['너비'] == 1016, '생산길이'].sum()
    ...
    def _handle_type2(self, sub_df: pd.DataFrame) -> dict:
        ...
        """
        # 필수 원단 존재 여부 확인
        has_609 = 609 in sub_df['너비'].values
        has_914 = 914 in sub_df['너비'].values
        ...
        if not has_914: # 914 없고 609만 있는 경우
            ...
            return {
                'fabric_width': 1500,
                'production_length': sub_df.loc[sub_df['너비'] == 609, '생산길이'].sum()
            }
        if not has_609: # 609 없고 914만 있는 경우
            print("원단 길이에 맞는 609mm의 주문이 없어서 914mm 단독처리")
            return {
                'fabric_width': 1500,
                'production_length': sub_df.loc[sub_df['너비'] == 914, '생산길이'].sum()
            }
        ...
        return {
            'fabric_width': 1500,
            'production_length': max(sub_df.loc[sub_df['너비'] == 609, '생산길이'].sum(), 
                                     sub_df.loc[sub_df['너비'] == 914, '생산길이'].sum())
        }
    ...
    def _handle_type3(self, sub_df: pd.DataFrame) -> dict:
        ...
        return {
            'fabric_width': 1500,
            'production_length': sub_df['생산길이'].sum() / 2
        }
...
class FabricCombiner:
    ...
    def __init__(self, groupby_cols=None, dropna=False):
        ...
        self.groupby_cols = groupby_cols or ['GITEM', '공정', '공정순서', '조합분류']
    ...
    def _add_combination_type(self, df: pd.DataFrame) -> pd.DataFrame:
        """조합분류 컬럼 추가 (너비 기준)"""
        df = df.copy()
        df['조합분류'] = df['너비'].apply(self.rule_handler.classify_width)
        return df 
    ...
    def _assign_fabric_quantity(self, sub_df: pd.DataFrame) -> pd.DataFrame:
        ...
        return pd.DataFrame([{
            'P/O NO': ', '.join(map(str, sub_df['P/O NO'].explode().unique())),
            'GITEM': sub_df['GITEM'].iloc[0],
            '공정': sub_df['공정'].iloc[0],
            '공정순서': sub_df['공정순서'].iloc[0],
            '조합분류': comb_type,
            '납기일': sub_df['납기일'].min(),
            '원단너비': result['fabric_width'],
            '생산길이': result['production_length'],
            '배합코드': sub_df['배합코드'].iloc[0],
