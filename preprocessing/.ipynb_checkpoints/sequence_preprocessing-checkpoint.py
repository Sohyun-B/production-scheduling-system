...
def process_operations_by_category(merged_df):
    ...
    for gitem in merged_df['GITEM'].dropna().unique():
        category_df = merged_df[merged_df['GITEM'] == gitem]

        for operation in category_df['공정'].dropna().unique():
            opslice = category_df[category_df['공정'] == operation]

            # 배합코드 유무에 따라 분리
            nan_bh = opslice[opslice['배합코드'].isna()]
            notnan_bh = opslice[~opslice['배합코드'].isna()]

            # 배합코드가 존재하는 경우
            if not notnan_bh.empty:
                groupby_col = ['GITEM', '공정', '배합코드', '조합분류']
                ...
                if not paired_order.empty:
                    paired_order['ID'] = (
                        str(gitem) + "_" + 
                        paired_order['공정'] + "_" +
                        paired_order['원단너비'].round().astype(int).astype(str) + "_" + 
                        paired_order['배합코드'].astype(str) + "_" +
                        paired_order['조합분류'].astype(str)
                    )
...
def create_sequence_seperated_order(order_list, operation_seperated_sequence):
    ...
    for order_df in order_list:
        # 병합 및 전처리 (외부에서 처리)
        merged = pd.merge(order_df, operation_seperated_sequence, on=['GITEM'], how='left')
        merged = merged.rename(columns={'원단길이': '생산길이'})
    ...
    # 해시 생성 후 ID에 추가
    sequence_seperated_order['ID'] = sequence_seperated_order['ID'].astype(str) + "_M" + sequence_seperated_order['납기일'].dt.month.astype(str)
    sequence_seperated_order['공정순서'] = sequence_seperated_order['공정순서'].astype(int)
