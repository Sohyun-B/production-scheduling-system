...
def process_operations_by_category(merged_df):
    ...
    for gitem in merged_df[config.columns.GITEM].dropna().unique():
        ...
        for operation in category_df[config.columns.OPERATION].dropna().unique():
            opslice = category_df[category_df[config.columns.OPERATION] == operation]
            ...
            if not notnan_bh.empty:
                groupby_col = [config.columns.GITEM, config.columns.OPERATION, config.columns.MIXTURE_CODE, config.columns.COMBINATION_CLASSIFICATION]
                ...
                if not paired_order.empty:
                    paired_order[config.columns.ID] = (
                        str(gitem) + "_" + 
                        paired_order[config.columns.OPERATION] + "_" +
