...
async def test_stage5_async():
    """5단계 스케줄링 비동기 테스트"""
    print("🚀 5단계 스케줄링 비동기 테스트 시작")
    print("=" * 50)
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                print(f"✅ 5단계 성공: 스케줄링 시작")
                print(f"📊 메시지: {result['message']}")
            else:
                print(f"❌ 5단계 실패: {response.status_code}")
                print(f"📊 오류 내용: {response.text}")
                return
            ...
            while attempt < max_attempts:
                attempt += 1
                print(f"🔄 상태 확인 {attempt}/{max_attempts}...")
                ...
                if response.status_code == 200:
                    ...
                    if status['status'] == 'completed':
                        print(f"✅ 스케줄링 완료!")
                        print(f"📊 스케줄링된 작업: {status['scheduled_jobs']}개")
                        print(f"📊 Makespan: {status['makespan']}시간")
                        print(f"📊 완료 시간: {status.get('completed_at', 'N/A')}")
                        break
                    elif status['status'] == 'failed':
                        print(f"❌ 스케줄링 실패: {status['message']}")
                        break
                    else:
                        print(f"⏳ 스케줄링 진행 중... 2초 후 다시 확인")
            ...
            if attempt >= max_attempts:
                print("⏰ 타임아웃: 스케줄링이 60초 내에 완료되지 않았습니다.")
            
            print("\n" + "=" * 50)
            print("🏁 5단계 비동기 테스트 완료")
